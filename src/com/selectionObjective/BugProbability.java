...
public class BugProbability {
	...
	public Object[] trainAndPredictProb ( String fileTrain, String fileTest, String classifyType ){
		try {
			/*
	...
	public void prepareWekaData ( TestProject project, ArrayList<TestProject> historyProjectList, HashMap<String, CrowdWorker> workerList ) {
		...
		ArrayList<TestCase> caseListTest = testCaseTool.prepareTestCaseInfo_wekaTest(project, workerList, defaultWorker);
	}
	
	public void generateWekaDataFile ( ArrayList<TestCase> caseList, String fileName ) {
		...
		try {
			...
			for ( int i =0;  i < caseList.size(); i++ ) {
				...
				if ( testCase.getTestOracle().equals( "审核通过")) {
					writer.write( "yes");
				}else {
					writer.write( "no");
				}
